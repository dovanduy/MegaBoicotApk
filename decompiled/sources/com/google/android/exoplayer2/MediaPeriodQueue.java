...
final class MediaPeriodQueue {
    ...
    public boolean shouldLoadNextMediaPeriod() {
        return this.loading == null || (!this.loading.info.isFinal && this.loading.isFullyBuffered() && this.loading.info.durationUs != C2793C.TIME_UNSET && this.length < 100);
    }
    ...
    public MediaPeriodHolder advancePlayingPeriod() {
        if (this.playing != null) {
            ...
            if (this.length == 0) {
                ...
                this.oldFrontPeriodWindowSequenceNumber = this.playing.info.f8627id.windowSequenceNumber;
            }
    ...
    public void clear(boolean z) {
        ...
        if (frontPeriod != null) {
            ...
            this.oldFrontPeriodWindowSequenceNumber = frontPeriod.info.f8627id.windowSequenceNumber;
            frontPeriod.release();
    ...
    public MediaPeriodInfo getUpdatedMediaPeriodInfo(MediaPeriodInfo mediaPeriodInfo, int i) {
        return getUpdatedMediaPeriodInfo(mediaPeriodInfo, mediaPeriodInfo.f8627id.copyWithPeriodIndex(i));
    }
    ...
    private long resolvePeriodIndexToWindowSequenceNumber(int i) {
        ...
        if (this.oldFrontPeriodUid != null) {
            int indexOfPeriod = this.timeline.getIndexOfPeriod(this.oldFrontPeriodUid);
        ...
        for (MediaPeriodHolder frontPeriod = getFrontPeriod(); frontPeriod != null; frontPeriod = frontPeriod.next) {
            if (frontPeriod.uid.equals(obj)) {
                return frontPeriod.info.f8627id.windowSequenceNumber;
            }
        }
        for (MediaPeriodHolder frontPeriod2 = getFrontPeriod(); frontPeriod2 != null; frontPeriod2 = frontPeriod2.next) {
            ...
            if (indexOfPeriod2 != -1 && this.timeline.getPeriod(indexOfPeriod2, this.period).windowIndex == i2) {
                return frontPeriod2.info.f8627id.windowSequenceNumber;
            }
        }
    ...
    private boolean canKeepMediaPeriodHolder(MediaPeriodHolder mediaPeriodHolder, MediaPeriodInfo mediaPeriodInfo) {
        ...
        return mediaPeriodInfo2.startPositionUs == mediaPeriodInfo.startPositionUs && mediaPeriodInfo2.endPositionUs == mediaPeriodInfo.endPositionUs && mediaPeriodInfo2.f8627id.equals(mediaPeriodInfo.f8627id);
    }
    ...
    private boolean updateForPlaybackModeChange() {
        ...
        while (true) {
            int nextPeriodIndex = this.timeline.getNextPeriodIndex(frontPeriod.info.f8627id.periodIndex, this.period, this.window, this.repeatMode, this.shuffleModeEnabled);
            while (frontPeriod.next != null && !frontPeriod.info.isLastInTimelinePeriod) {
                frontPeriod = frontPeriod.next;
            }
            if (nextPeriodIndex == -1 || frontPeriod.next == null || frontPeriod.next.info.f8627id.periodIndex != nextPeriodIndex) {
                boolean removeAfter = removeAfter(frontPeriod);
                frontPeriod.info = getUpdatedMediaPeriodInfo(frontPeriod.info, frontPeriod.info.f8627id);
            } else {
                frontPeriod = frontPeriod.next;
            }
        }
    ...
    private MediaPeriodInfo getFollowingMediaPeriodInfo(MediaPeriodHolder mediaPeriodHolder, long j) {
        ...
        if (mediaPeriodInfo.isLastInTimelinePeriod) {
            int nextPeriodIndex = this.timeline.getNextPeriodIndex(mediaPeriodInfo.f8627id.periodIndex, this.period, this.window, this.repeatMode, this.shuffleModeEnabled);
            if (nextPeriodIndex == -1) {
                return null;
            }
            int i2 = this.timeline.getPeriod(nextPeriodIndex, this.period, true).windowIndex;
            Object obj = this.period.uid;
            long j4 = mediaPeriodInfo.f8627id.windowSequenceNumber;
            long j5 = 0;
            if (this.timeline.getWindow(i2, this.window).firstPeriodIndex == nextPeriodIndex) {
                long rendererOffset = (mediaPeriodHolder.getRendererOffset() + mediaPeriodInfo.durationUs) - j;
                Timeline timeline2 = this.timeline;
                Pair periodPosition = timeline2.getPeriodPosition(this.window, this.period, i2, C2793C.TIME_UNSET, Math.max(0, rendererOffset));
                ...
                if (mediaPeriodHolder2.next == null || !mediaPeriodHolder2.next.uid.equals(obj)) {
                    ...
                    j3 = mediaPeriodHolder2.next.info.f8627id.windowSequenceNumber;
                }
                j5 = longValue;
                j2 = j3;
                i = intValue;
        ...
        MediaPeriodId mediaPeriodId = mediaPeriodInfo.f8627id;
    ...
    private MediaPeriodInfo getMediaPeriodInfoForContent(int i, long j, long j2) {
        ...
        boolean isLastInPeriod = isLastInPeriod(mediaPeriodId, j3);
        MediaPeriodInfo mediaPeriodInfo = new MediaPeriodInfo(mediaPeriodId, j4, j3, C2793C.TIME_UNSET, j3 == Long.MIN_VALUE ? this.period.getDurationUs() : j3, isLastInPeriod, isLastInTimeline(mediaPeriodId, isLastInPeriod));
        return mediaPeriodInfo;
    }
