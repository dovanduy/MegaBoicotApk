...
final class DashMediaPeriod implements MediaPeriod, Callback<ChunkSampleStream<DashChunkSource>>, ReleaseCallback<DashChunkSource> {
    ...
    public DashMediaPeriod(int i, DashManifest dashManifest, int i2, Factory factory, int i3, EventDispatcher eventDispatcher2, long j, LoaderErrorThrower loaderErrorThrower, Allocator allocator2, CompositeSequenceableLoaderFactory compositeSequenceableLoaderFactory2, PlayerEmsgCallback playerEmsgCallback) {
        this.f8646id = i;
    ...
    public void updateManifest(DashManifest dashManifest, int i) {
        ...
        if (this.periodIndex != i) {
            this.eventDispatcher = this.eventDispatcher.withParameters(0, this.eventDispatcher.mediaPeriodId.copyWithPeriodIndex(i), dashManifest.getPeriod(i).startMs);
        }
        this.periodIndex = i;
        ...
        for (EventSampleStream eventSampleStream : this.eventSampleStreams) {
            ...
            while (true) {
                ...
                if (eventStream.mo11222id().equals(eventSampleStream.eventStreamId())) {
                    boolean z = true;
    ...
    private static int[][] getGroupedAdaptationSetIndices(List<AdaptationSet> list) {
        ...
        for (int i = 0; i < size; i++) {
            sparseIntArray.put(((AdaptationSet) list.get(i)).f8647id, i);
        }
    ...
    private static int buildPrimaryAndEmbeddedTrackGroupInfos(List<AdaptationSet> list, int[][] iArr, int i, boolean[] zArr, boolean[] zArr2, TrackGroup[] trackGroupArr, TrackGroupInfo[] trackGroupInfoArr) {
        ...
        while (i9 < i8) {
            ...
            for (int i11 = i7; i11 < length; i11++) {
                arrayList.addAll(((AdaptationSet) list2.get(iArr2[i11])).representations);
            }
            Format[] formatArr = new Format[arrayList.size()];
            for (int i12 = i7; i12 < formatArr.length; i12++) {
                formatArr[i12] = ((Representation) arrayList.get(i12)).format;
            }
            AdaptationSet adaptationSet = (AdaptationSet) list2.get(iArr2[i7]);
            int i13 = i10 + 1;
            if (zArr[i9]) {
                i2 = i13 + 1;
                i3 = i13;
            ...
            trackGroupInfoArr[i10] = TrackGroupInfo.primaryTrack(adaptationSet.type, iArr2, i10, i3, i2);
            if (i3 != -1) {
                StringBuilder sb = new StringBuilder();
                sb.append(adaptationSet.f8647id);
                sb.append(":emsg");
                trackGroupArr[i3] = new TrackGroup(Format.createSampleFormat(sb.toString(), MimeTypes.APPLICATION_EMSG, null, -1, null));
                trackGroupInfoArr[i3] = TrackGroupInfo.embeddedEmsgTrack(iArr2, i10);
                i5 = -1;
            } else {
                i5 = -1;
            }
    ...
    private static void buildManifestEventTrackGroupInfos(List<EventStream> list, TrackGroup[] trackGroupArr, TrackGroupInfo[] trackGroupInfoArr, int i) {
        ...
        int i3 = 0;
        while (i3 < list.size()) {
            trackGroupArr[i2] = new TrackGroup(Format.createSampleFormat(((EventStream) list.get(i3)).mo11222id(), MimeTypes.APPLICATION_EMSG, null, -1, null));
            int i4 = i2 + 1;
