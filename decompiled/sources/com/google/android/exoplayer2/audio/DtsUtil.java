...
public final class DtsUtil {
    ...
    private static final int[] TWICE_BITRATE_KBPS_BY_RATE = {64, 112, 128, PsExtractor.AUDIO_STREAM, 224, 256, 384, 448, 512, 640, 768, 896, C1245d.f3947iP, 1152, 1280, 1536, 1920, 2048, 2304, 2560, 2688, 2816, 2823, 2944, 3072, 3840, MpegAudioHeader.MAX_FRAME_SIZE_BYTES, 6144, 7680};
    ...
    public static Format parseDtsFormat(byte[] bArr, String str, String str2, DrmInitData drmInitData) {
        ...
        int i3 = readBits >= TWICE_BITRATE_KBPS_BY_RATE.length ? -1 : (TWICE_BITRATE_KBPS_BY_RATE[readBits] * AdError.NETWORK_ERROR_CODE) / 2;
    ...
    public static int getDtsFrameSize(byte[] bArr) {
        ...
        if (b != 31) {
            switch (b) {
                case -2:
                    i = (((bArr[6] & 240) >> 4) | ((bArr[4] & 3) << 12) | ((bArr[7] & FIRST_BYTE_14B_LE) << 4)) + 1;
                    break;
                case -1:
                    i = (((bArr[9] & 60) >> 2) | ((bArr[7] & 3) << 12) | ((bArr[6] & FIRST_BYTE_14B_LE) << 4)) + 1;
                    break;
                default:
                    i = (((bArr[7] & 240) >> 4) | ((bArr[5] & 3) << 12) | ((bArr[6] & FIRST_BYTE_14B_LE) << 4)) + 1;
                    break;
            }
        } else {
            i = (((bArr[8] & 60) >> 2) | ((bArr[6] & 3) << 12) | ((bArr[7] & FIRST_BYTE_14B_LE) << 4)) + 1;
        }
        z = true;
        return z ? (i * 16) / 14 : i;
    }
