package com.google.android.gms.internal.ads;

import android.app.Activity;
import android.content.Context;
import android.text.TextUtils;
import android.util.DisplayMetrics;
import android.util.Log;
import android.view.MotionEvent;
import android.view.View;
import java.lang.reflect.InvocationTargetException;
import java.lang.reflect.Method;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.concurrent.Callable;
import java.util.concurrent.ExecutorService;
import java.util.concurrent.TimeUnit;

public class agu extends agt {

    /* renamed from: s */
    private static final Object f11018s = new Object();

    /* renamed from: t */
    private static final String f11019t = "agu";

    /* renamed from: u */
    private static boolean f11020u = false;

    /* renamed from: v */
    private static long f11021v;

    /* renamed from: r */
    protected boolean f11022r = false;

    /* renamed from: w */
    private String f11023w;

    /* renamed from: x */
    private boolean f11024x = false;

    /* renamed from: y */
    private boolean f11025y = false;

    /* renamed from: z */
    private ahx f11026z;

    protected agu(Context context, String str, boolean z) {
        super(context);
        this.f11023w = str;
        this.f11022r = z;
    }

    /* renamed from: a */
    private static ahu m14082a(ahn ahn, MotionEvent motionEvent, DisplayMetrics displayMetrics) throws ahk {
        Method a = ahn.mo14302a("ryulhTIO44CH2QVINEPOtPkpQOSdbbq7HgisLeHH33VCs+TjwYWzYttvzJcQiI1k", "dnKLchXhJgI3U7L4th0PQJWe3DOIJJ7MUji5p7t5mjc=");
        if (a == null || motionEvent == null) {
            throw new ahk();
        }
        try {
            return new ahu((String) a.invoke(null, new Object[]{motionEvent, displayMetrics}));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new ahk(e);
        }
    }

    /* renamed from: a */
    protected static synchronized void m14083a(Context context, boolean z) {
        synchronized (agu.class) {
            if (!f11020u) {
                f11021v = System.currentTimeMillis() / 1000;
                f10998a = m14085b(context, z);
                f11020u = true;
            }
        }
    }

    /* renamed from: a */
    private static void m14084a(List<Callable<Void>> list) {
        if (f10998a != null) {
            ExecutorService c = f10998a.mo14307c();
            if (c != null && !list.isEmpty()) {
                try {
                    c.invokeAll(list, ((Long) aoq.m14620f().mo14695a(aru.f11832bD)).longValue(), TimeUnit.MILLISECONDS);
                } catch (InterruptedException e) {
                    Log.d(f11019t, String.format("class methods got exception: %s", new Object[]{ahv.m14161a((Throwable) e)}));
                }
            }
        }
    }

    /* renamed from: b */
    private static ahn m14085b(Context context, boolean z) {
        if (f10998a == null) {
            synchronized (f11018s) {
                if (f10998a == null) {
                    ahn a = ahn.m14121a(context, "fFhi0cTZpyVQYwMwl7BCfa0fa6esmkRUaNr4ktfJYZ8=", "", z);
                    if (a.mo14306b()) {
                        if (((Boolean) aoq.m14620f().mo14695a(aru.f11838bJ)).booleanValue()) {
                            a.mo14304a("4o7tecxtkw7XaNt5hPj+0H1LvOi0SgxCIJTY9VcbazM/HSl/sFlxBFwnc8glnvoB", "RgSY6YxU2k1vLXOV3vapBnQwJDzYDlmX50wbm2tDcnw=", new Class[0]);
                        }
                        a.mo14304a("1QeH3Cf7T53ayw17Ebbo9YTdhU+IFx0X5nCtC5gZQym4uicOVPXxYWmMK9k58i8n", "bHJRpFJ+2R5LAbYQUBDMyfYpLd1oiGixlpIqMJOBQPY=", Context.class);
                        a.mo14304a("1DzTSPdcTVkbB0cTQEaPo/MU1E1TGypt38nYJILt/8CcQbOuYjF6hLShUHnLb5uw", "dkCo8xze5sFyp7Msrb8F1JKjVIP7LutspgZPYQvsMlo=", Context.class);
                        a.mo14304a("+8WN7RfVBrTKK5NXM+y9JA4Ygu2jPZokHNFGgulhBRZ32uCnyKWWGQNTE1weoQCM", "peILYfFejTn8xmazfpiZWeK2eaYjjxEBmAZ8bUD9PWU=", Context.class);
                        a.mo14304a("6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", Context.class);
                        a.mo14304a("f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", Context.class);
                        a.mo14304a("bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", Context.class);
                        a.mo14304a("ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", Context.class);
                        a.mo14304a("PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", Context.class);
                        a.mo14304a("ryulhTIO44CH2QVINEPOtPkpQOSdbbq7HgisLeHH33VCs+TjwYWzYttvzJcQiI1k", "dnKLchXhJgI3U7L4th0PQJWe3DOIJJ7MUji5p7t5mjc=", MotionEvent.class, DisplayMetrics.class);
                        a.mo14304a("oT49Nu6m3oNdAQwbQzvwr/RE4TwZbR8mJKypakrI/GqBgyD2niyVic8N9UaqvU10", "LS3Cw7DlWSqpRsXrajoVsCxXOedCBnQXFZcLjXzD8so=", MotionEvent.class, DisplayMetrics.class);
                        a.mo14304a("1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", new Class[0]);
                        a.mo14304a("R1L5/7J00Fko/kDi6VDsBhCzpp4/7wzabnzdihUS7rPLvaXLFWrERChyvYYgVJRF", "4WtEOgM4wQ7wiw1OB3pMyAbaplBizcjE6LRBT/nrDgM=", new Class[0]);
                        a.mo14304a("uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", new Class[0]);
                        a.mo14304a("C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", new Class[0]);
                        a.mo14304a("YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", new Class[0]);
                        a.mo14304a("Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", new Class[0]);
                        a.mo14304a("LJD7GILw6ox/o3kYqhR5FF4FDylErrAaJ66Y8V8gtnNGF8DcetGJrYvTlHeK8kyR", "GKdCM/OywOIR4FMFS5XrDdTIpVvhoMFma/KSfRUdsbQ=", Context.class, Boolean.TYPE, Boolean.TYPE);
                        a.mo14304a("nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=", StackTraceElement[].class);
                        a.mo14304a("j5/WVRJsgiWNVIg0a6aXYQYocHm4rEEozf0LrIkPezct93ISwgyVBTLRIkEvztpn", "gCYklRkrI0pyKJ3j1u5Yl6Jj12a6PD1K39diuw7+f7o=", View.class, DisplayMetrics.class);
                        a.mo14304a("WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", Context.class, Boolean.TYPE);
                        a.mo14304a("wxabFp8V9bGKut15duzoi2M8ew0t/HWGFw3NoPp8pFanKwQpdIx0WzpFbmJaLX9T", "grSrEy+Ml8J7boJNrN9uNiXpddsqwR6A9NVesV/hHT8=", View.class, Activity.class, Boolean.TYPE);
                        a.mo14304a("aH422z6bUtRL57gOABo0AU5IQDHmWvjsQgQZk64bzhjXVYUZmajAsUz/rrnMYVsw", "UAmRxgiWCEqHprf42V+e65VvIvUfFXeXbaMy5B8vzwA=", Long.TYPE);
                    }
                    f10998a = a;
                }
            }
        }
        return f10998a;
    }

    /* access modifiers changed from: protected */
    /* renamed from: a */
    public final long mo14279a(StackTraceElement[] stackTraceElementArr) throws ahk {
        Method a = f10998a.mo14302a("nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=");
        if (a == null || stackTraceElementArr == null) {
            throw new ahk();
        }
        try {
            return new ahl((String) a.invoke(null, new Object[]{stackTraceElementArr})).f11051a.longValue();
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new ahk(e);
        }
    }

    /* access modifiers changed from: protected */
    /* renamed from: a */
    public final C4378zo mo14280a(Context context, View view, Activity activity) {
        List list;
        C4378zo zoVar = new C4378zo();
        if (!TextUtils.isEmpty(this.f11023w)) {
            zoVar.f14330b = this.f11023w;
        }
        ahn b = m14085b(context, this.f11022r);
        if (!b.mo14306b()) {
            zoVar.f14339k = Long.valueOf(16384);
            list = Arrays.asList(new Callable[]{new aii(b, zoVar)});
        } else {
            try {
                ahu a = m14082a(b, this.f10999b, this.f11014q);
                zoVar.f14335g = a.f11090a;
                zoVar.f14336h = a.f11091b;
                zoVar.f14337i = a.f11092c;
                if (this.f11013p) {
                    zoVar.f14349u = a.f11093d;
                    zoVar.f14350v = a.f11094e;
                }
            } catch (ahk unused) {
            }
            aap aap = new aap();
            if (this.f11001d > 0 && ahv.m14163a(this.f11014q)) {
                aap.f10319m = Long.valueOf(ahv.m14158a(this.f11008k, this.f11014q));
                aap.f10320n = Long.valueOf(ahv.m14158a((double) (this.f11011n - this.f11009l), this.f11014q));
                aap.f10321o = Long.valueOf(ahv.m14158a((double) (this.f11012o - this.f11010m), this.f11014q));
                aap.f10324r = Long.valueOf(ahv.m14158a((double) this.f11009l, this.f11014q));
                aap.f10325s = Long.valueOf(ahv.m14158a((double) this.f11010m, this.f11014q));
                if (this.f11013p && this.f10999b != null) {
                    long a2 = ahv.m14158a((double) (((this.f11009l - this.f11011n) + this.f10999b.getRawX()) - this.f10999b.getX()), this.f11014q);
                    if (a2 != 0) {
                        aap.f10322p = Long.valueOf(a2);
                    }
                    long a3 = ahv.m14158a((double) (((this.f11010m - this.f11012o) + this.f10999b.getRawY()) - this.f10999b.getY()), this.f11014q);
                    if (a3 != 0) {
                        aap.f10323q = Long.valueOf(a3);
                    }
                }
            }
            try {
                ahu b2 = mo14282b(this.f10999b);
                aap.f10307a = b2.f11090a;
                aap.f10308b = b2.f11091b;
                aap.f10314h = b2.f11092c;
                if (this.f11013p) {
                    aap.f10309c = b2.f11094e;
                    aap.f10311e = b2.f11093d;
                    aap.f10313g = Integer.valueOf(b2.f11095f.longValue() != 0 ? 1 : 0);
                    if (this.f11002e > 0) {
                        aap.f10310d = ahv.m14163a(this.f11014q) ? Long.valueOf(Math.round(((double) this.f11007j) / ((double) this.f11002e))) : null;
                        aap.f10312f = Long.valueOf(Math.round(((double) this.f11006i) / ((double) this.f11002e)));
                    }
                    aap.f10316j = b2.f11098i;
                    aap.f10315i = b2.f11099j;
                    aap.f10317k = Integer.valueOf(b2.f11100k.longValue() != 0 ? 1 : 0);
                }
            } catch (ahk unused2) {
            }
            if (this.f11005h > 0) {
                aap.f10318l = Long.valueOf(this.f11005h);
            }
            zoVar.f14296K = aap;
            if (this.f11001d > 0) {
                zoVar.f14353y = Long.valueOf(this.f11001d);
            }
            if (this.f11002e > 0) {
                zoVar.f14352x = Long.valueOf(this.f11002e);
            }
            if (this.f11003f > 0) {
                zoVar.f14351w = Long.valueOf(this.f11003f);
            }
            if (this.f11004g > 0) {
                zoVar.f14354z = Long.valueOf(this.f11004g);
            }
            try {
                int size = this.f11000c.size() - 1;
                if (size > 0) {
                    zoVar.f14297L = new aap[size];
                    for (int i = 0; i < size; i++) {
                        ahu a4 = m14082a(f10998a, (MotionEvent) this.f11000c.get(i), this.f11014q);
                        aap aap2 = new aap();
                        aap2.f10307a = a4.f11090a;
                        aap2.f10308b = a4.f11091b;
                        zoVar.f14297L[i] = aap2;
                    }
                }
            } catch (ahk unused3) {
                zoVar.f14297L = null;
            }
            list = new ArrayList();
            if (b.mo14307c() != null) {
                int n = b.mo14318n();
                list.add(new aii(b, zoVar));
                ahn ahn = b;
                C4378zo zoVar2 = zoVar;
                ail ail = new ail(ahn, "uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", zoVar2, n, 1);
                list.add(ail);
                aig aig = new aig(ahn, "1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", zoVar2, f11021v, n, 25);
                list.add(aig);
                int i2 = n;
                aif aif = new aif(ahn, "C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", zoVar2, i2, 44);
                list.add(aif);
                aik aik = new aik(ahn, "f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", zoVar2, i2, 12);
                list.add(aik);
                aim aim = new aim(ahn, "bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", zoVar2, i2, 3);
                list.add(aim);
                aij aij = new aij(ahn, "YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", zoVar2, i2, 22);
                list.add(aij);
                aie aie = new aie(ahn, "6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", zoVar2, i2, 5);
                list.add(aie);
                aiu aiu = new aiu(ahn, "ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", zoVar2, i2, 48);
                list.add(aiu);
                aib aib = new aib(ahn, "PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", zoVar2, i2, 49);
                list.add(aib);
                air air = new air(ahn, "Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", zoVar2, i2, 51);
                list.add(air);
                aiq aiq = new aiq(ahn, "nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=", zoVar2, i2, 45, new Throwable().getStackTrace());
                list.add(aiq);
                aiv aiv = new aiv(ahn, "j5/WVRJsgiWNVIg0a6aXYQYocHm4rEEozf0LrIkPezct93ISwgyVBTLRIkEvztpn", "gCYklRkrI0pyKJ3j1u5Yl6Jj12a6PD1K39diuw7+f7o=", zoVar2, i2, 57, view);
                list.add(aiv);
                aio aio = new aio(ahn, "WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", zoVar2, i2, 61);
                list.add(aio);
                if (((Boolean) aoq.m14620f().mo14695a(aru.f11833bE)).booleanValue()) {
                    aia aia = new aia(b, "wxabFp8V9bGKut15duzoi2M8ew0t/HWGFw3NoPp8pFanKwQpdIx0WzpFbmJaLX9T", "grSrEy+Ml8J7boJNrN9uNiXpddsqwR6A9NVesV/hHT8=", zoVar, n, 62, view, activity);
                    list.add(aia);
                }
                if (((Boolean) aoq.m14620f().mo14695a(aru.f11835bG)).booleanValue()) {
                    ais ais = new ais(b, "aH422z6bUtRL57gOABo0AU5IQDHmWvjsQgQZk64bzhjXVYUZmajAsUz/rrnMYVsw", "UAmRxgiWCEqHprf42V+e65VvIvUfFXeXbaMy5B8vzwA=", zoVar, n, 53, this.f11026z);
                    list.add(ais);
                }
            }
        }
        m14084a(list);
        return zoVar;
    }

    /* access modifiers changed from: protected */
    /* renamed from: a */
    public final C4378zo mo14281a(Context context, C4274wk wkVar) {
        C4378zo zoVar = new C4378zo();
        if (!TextUtils.isEmpty(this.f11023w)) {
            zoVar.f14330b = this.f11023w;
        }
        ahn b = m14085b(context, this.f11022r);
        if (b.mo14307c() != null) {
            m14084a(mo14283a(b, zoVar, (C4274wk) null));
        }
        return zoVar;
    }

    /* access modifiers changed from: protected */
    /* renamed from: a */
    public List<Callable<Void>> mo14283a(ahn ahn, C4378zo zoVar, C4274wk wkVar) {
        int n = ahn.mo14318n();
        ArrayList arrayList = new ArrayList();
        if (!ahn.mo14306b()) {
            zoVar.f14339k = Long.valueOf(16384);
            return arrayList;
        }
        ahn ahn2 = ahn;
        C4378zo zoVar2 = zoVar;
        aid aid = new aid(ahn2, "LJD7GILw6ox/o3kYqhR5FF4FDylErrAaJ66Y8V8gtnNGF8DcetGJrYvTlHeK8kyR", "GKdCM/OywOIR4FMFS5XrDdTIpVvhoMFma/KSfRUdsbQ=", zoVar2, n, 27, wkVar);
        arrayList.add(aid);
        aig aig = new aig(ahn2, "1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", zoVar2, f11021v, n, 25);
        arrayList.add(aig);
        int i = n;
        ail ail = new ail(ahn2, "uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", zoVar2, i, 1);
        arrayList.add(ail);
        ain ain = new ain(ahn2, "+8WN7RfVBrTKK5NXM+y9JA4Ygu2jPZokHNFGgulhBRZ32uCnyKWWGQNTE1weoQCM", "peILYfFejTn8xmazfpiZWeK2eaYjjxEBmAZ8bUD9PWU=", zoVar2, i, 31);
        arrayList.add(ain);
        ait ait = new ait(ahn2, "R1L5/7J00Fko/kDi6VDsBhCzpp4/7wzabnzdihUS7rPLvaXLFWrERChyvYYgVJRF", "4WtEOgM4wQ7wiw1OB3pMyAbaplBizcjE6LRBT/nrDgM=", zoVar2, i, 33);
        arrayList.add(ait);
        aic aic = new aic(ahn2, "1DzTSPdcTVkbB0cTQEaPo/MU1E1TGypt38nYJILt/8CcQbOuYjF6hLShUHnLb5uw", "dkCo8xze5sFyp7Msrb8F1JKjVIP7LutspgZPYQvsMlo=", zoVar2, i, 29);
        arrayList.add(aic);
        aie aie = new aie(ahn2, "6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", zoVar2, i, 5);
        arrayList.add(aie);
        aik aik = new aik(ahn2, "f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", zoVar2, i, 12);
        arrayList.add(aik);
        aim aim = new aim(ahn2, "bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", zoVar2, i, 3);
        arrayList.add(aim);
        aif aif = new aif(ahn2, "C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", zoVar2, i, 44);
        arrayList.add(aif);
        aij aij = new aij(ahn2, "YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", zoVar2, i, 22);
        arrayList.add(aij);
        aiu aiu = new aiu(ahn2, "ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", zoVar2, i, 48);
        arrayList.add(aiu);
        aib aib = new aib(ahn2, "PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", zoVar2, i, 49);
        arrayList.add(aib);
        air air = new air(ahn2, "Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", zoVar2, i, 51);
        arrayList.add(air);
        aio aio = new aio(ahn2, "WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", zoVar2, i, 61);
        arrayList.add(aio);
        return arrayList;
    }

    /* renamed from: a */
    public final void mo12629a(View view) {
        if (((Boolean) aoq.m14620f().mo14695a(aru.f11835bG)).booleanValue()) {
            if (this.f11026z == null) {
                this.f11026z = new ahx(f10998a, view);
            } else {
                this.f11026z.mo14327a(view);
            }
        }
    }

    /* access modifiers changed from: protected */
    /* renamed from: b */
    public final ahu mo14282b(MotionEvent motionEvent) throws ahk {
        Method a = f10998a.mo14302a("oT49Nu6m3oNdAQwbQzvwr/RE4TwZbR8mJKypakrI/GqBgyD2niyVic8N9UaqvU10", "LS3Cw7DlWSqpRsXrajoVsCxXOedCBnQXFZcLjXzD8so=");
        if (a == null || motionEvent == null) {
            throw new ahk();
        }
        try {
            return new ahu((String) a.invoke(null, new Object[]{motionEvent, this.f11014q}));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new ahk(e);
        }
    }
}
