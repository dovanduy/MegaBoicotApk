...
public final class SsMediaSource extends BaseMediaSource implements Callback<ParsingLoadable<SsManifest>> {
    ...
    private SsMediaSource(SsManifest ssManifest, Uri uri, com.google.android.exoplayer2.upstream.DataSource.Factory factory, Parser<? extends SsManifest> parser, com.google.android.exoplayer2.source.smoothstreaming.SsChunkSource.Factory factory2, CompositeSequenceableLoaderFactory compositeSequenceableLoaderFactory2, int i, long j, Object obj) {
        Uri uri2;
        boolean z = true;
    ...
    private void processManifest() {
        ...
        if (j2 == Long.MAX_VALUE) {
            ...
            if (this.manifest.dvrWindowLengthUs != C2793C.TIME_UNSET && this.manifest.dvrWindowLengthUs > 0) {
                j2 = Math.max(j2, j - this.manifest.dvrWindowLengthUs);
            }
            long j3 = j2;
            long j4 = j - j3;
            long msToUs = j4 - C2793C.msToUs(this.livePresentationDelayMs);
            singlePeriodTimeline = new SinglePeriodTimeline(C2793C.TIME_UNSET, j4, j3, msToUs < MIN_LIVE_DEFAULT_START_POSITION_US ? Math.min(MIN_LIVE_DEFAULT_START_POSITION_US, j4 / 2) : msToUs, true, true, this.tag);
        } else {
            long j5 = this.manifest.durationUs != C2793C.TIME_UNSET ? this.manifest.durationUs : j - j2;
